...
'''class TinyTokenizer:
    def __init__(self, bos_token_id:int=2, vocab_size: int=3):
        self.bos_token_id=bos_token_id
        self.vocab_size=vocab_size

    def encode_ids(self, ids: List[int],add_bos: book=True)-> List[int]:
        if add_bos:
            return [self.bos_token_id]+ids
        else:
            return ids
    def decode_ids(self, ids: List[int], remove_bos: bool=True)-> List[int]:
        return ids

    def __len__(self):
        return self.vocab_size

Tokenizer=TinyTokenizer(bos_token_id=2,vocab_size=3)   '''        
BOS_ID=2
...
class MarkovData(Dataset):
    ...
    def __init__(
        ...
        self.raw_d_vocab = self.model.V
        self.prepend_bos=prepend_bos
        self.d_vocab = d_vocab + int(prepend_bos)
        ...
        for i in range(n_gen):
            ...
            if self.prepend_bos:
                seq=[BOS_ID]+list(tokens)

            else :
                seq=list(tokens)    
            self.data.append(torch.tensor(seq, dtype=torch.int64))
...
def train_model(
    ...
    n_ctx = dataset.gen_len +(1 if getattr(dataset, "prepend_bos",True)else 0)
