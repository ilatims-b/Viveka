...
def load_and_reduce_activations(save_base: str, start: int, target_dim: int = 512) -> Dict[int, t.Tensor]:
    ...
    for filename in os.listdir(save_base):
        if filename.endswith(f"_{start}.pt") and filename.startswith("layer_"):
            ...
            # Load activation tensor of shape (30, 2304)
            acts = t.load(path)
            ...
            reduced_acts[layer_idx] = reduced.float()
